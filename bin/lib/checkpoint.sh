#!/bin/bash
# GSD-VGL Ralph - Git Checkpointing
# Adapted from GSD to use plan.yaml task IDs (e.g., "1.1")
#
# Provides:
...
create_checkpoint_commit() {
    ...
    # Stage plan.yaml (tracks task status) and any modified files
    git add .claude/plan/plan.yaml 2>/dev/null
    ...
    local commit_msg
    commit_msg="checkpoint(task-${task_id}): ${summary}"
...
# Searches for most recent checkpoint commit
# Returns: task ID (e.g., "1.1") or empty string if no checkpoints
# Return code: 0 (always succeeds)
get_last_checkpoint_task() {
    # Search for most recent checkpoint commit (new or legacy format)
    local commit_line
    commit_line=$(git log --oneline --grep="checkpoint(task-" -1 2>/dev/null)

    # Fall back to legacy GSD format
    ...
    # Extract task ID - try new format first: checkpoint(task-1.1)
    local task_id
    task_id=$(echo "$commit_line" | grep -oP 'checkpoint\(task-\K[0-9]+\.[0-9]+' | head -1)

    # Fall back to legacy format: NN-MM
    if [[ -z "$task_id" ]]; then
...
validate_state_against_history() {
    ...
    # Extract phase and task numbers for comparison (format: N.M)
    local state_phase="${state_task%%.*}"
    local state_plan="${state_task##*.}"
    local git_phase="${git_task%%.*}"
    local git_plan="${git_task##*.}"

    # Numeric comparison (handles both N.M and NN-MM formats)
