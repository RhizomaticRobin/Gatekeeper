...
# get_plans_completed - Count completed plans from ROADMAP.md (falls back to plan.yaml)
...
get_plans_completed() {
    ...
    local plan="${PLAN_FILE:-.claude/plan/plan.yaml}"

    if [[ ! -f "$roadmap" ]]; then
        # Fall back to plan.yaml task completion count
        if [[ -f "$plan" ]]; then
            if [[ -z "${_ROADMAP_WARN_SHOWN:-}" ]]; then
                echo -e "${STATE_YELLOW}Warning: ROADMAP.md not found, using plan.yaml for progress${STATE_RESET}" >&2
                export _ROADMAP_WARN_SHOWN=1
            fi
            local counts
            counts=$(count_tasks 2>/dev/null)
            echo "${counts##* }"  # second field: completed
            return 0
        fi
        echo "0"
        return 1
...
# get_total_plans - Count total plans from ROADMAP.md (falls back to plan.yaml)
...
get_total_plans() {
    ...
    if [[ ! -f "$roadmap" ]]; then
        # Fall back to plan.yaml total task count
        if [[ -f "$plan" ]]; then
            local counts
            counts=$(count_tasks 2>/dev/null)
            echo "${counts%% *}"  # first field: total
            return 0
        fi
        echo "0"
        return 1
