...
4. **Verify the visual verification surface works**: Start the dev server and confirm the task's `playwright_url` endpoint serves a page with live data from the backend — not static HTML, not hardcoded values, not console dumps. The verifier will run Playwright against this URL and FAIL if the frontend is deceptive or disconnected.
...
Do not read files outside this scope. In particular, `.claude/` state files, `.claude/plugins/`, `.claude/gk-sessions/`, `gatekeeper/`, `gatekeeper-evolve-mcp/`, `verifier-mcp/`, `scripts/`, `agents/`, `hooks/`, and `commands/` are infrastructure managed by the system and not relevant to your implementation work.
