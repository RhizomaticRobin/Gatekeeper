...
```bash
# Each truth is a testable assertion
# Example truth: "All API endpoints return JSON responses"
# Verify by checking route handlers
grep -E "Response\.json|res\.json" src/api/**/*.ts

# Example truth: "Auth middleware protects all /admin routes"
# Verify by checking middleware wiring
grep -r "auth" src/app/admin/ --include="*.ts" -l
```

**Pass criteria:** Every truth must be demonstrably true. If a truth cannot be verified programmatically, flag for human verification.

### artifacts
Files and code that must exist as outputs of the task.
...
# Check artifacts are substantive (not stubs)
for artifact in "${artifacts[@]}"; do
  stubs=$(grep -c -E "TODO|FIXME|placeholder|not implemented" "$artifact" 2>/dev/null || echo 0)
  [ "$stubs" -gt 0 ] && echo "STUB_PATTERNS: $stubs in $artifact"
done
```

**Pass criteria:** All listed artifacts exist and pass stub detection checks.

### key_links
Wiring connections between components that must be established.

**Verification approach:**
```bash
# Each key_link specifies a source → target connection
# Example: "UserForm → POST /api/users"
grep -E "fetch.*\/api\/users|axios.*\/api\/users" src/components/UserForm.tsx

# Example: "middleware → auth.verify()"
grep -E "auth\.verify|verifyAuth" src/middleware.ts
```

**Pass criteria:** Every key_link can be traced through the code. Source references target, and target is reachable.

### Aggregate Verification
A task passes must_haves verification when:
- ALL truths are confirmed true
- ALL artifacts exist and are substantive
- ALL key_links are wired and traceable

If any category has failures, the task is NOT complete.
